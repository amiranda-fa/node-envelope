...
qp.encode = function (input, wordMode) {
  var bytes = !Buffer.isBuffer(input) ? Buffer.from(input) : input
...
qp.decode = function (input, charset, wordMode) {
  ...
  var byteLength =
    Buffer.byteLength(input) - (input.match(/=[0-9A-F]{2}/gi) || []).length * 2
...
qp.encodeWord = function (input, type, charset) {
  ...
  switch (type) {
    case 'B':
      input = qp.encodeBase64(input, charset)
      break
    case 'Q':
      input = qp.encode(input, true)
      break
    default:
